...
class Matrix(list):
    ...
    def __str__(self):
        ...
        """Returns a matrix"""
        return '\n'.join('\t'.join(str(line[column]) for column in range(len(line))) for line in self)
    ...
    def __add__(self, added_matix):
        ...
        uns = Matrix([[pair[0] + pair[1]\
           for pair in zip(self[line], added_matix[line])]\
    ...
    def transposed(matrix):
        ...
        return Matrix([[matrix[line][column]\
           for line in range(matrix.get_line)]\
          for column in range(matrix.get_column)])
    ...
    def __mul__ (self, second_mul):
        ...
        if (type(second_mul) in [int, float]):
            """Multiplication mutrix to number"""
            return Matrix(list(map(lambda line:\
               list(map(lambda point:\
              point * second_mul, line)),self)))
        elif (type(second_mul) in [Matrix,SquareMatrix]):
            """Multiplication mutrix to mutrix"""
            if self.get_column != second_mul.get_line:
                raise MatrixError(self, second_mul)
            uns = Matrix([[sum([second_mul[_][column]\
               * self[line][_] for _ in range(self.get_column)])\
...
class SquareMatrix (Matrix):
    ...
    def __pow__(self, num):
        ...
        if num == 0:
            return SquareMatrix([[0 if i != j else 1\
               for i in range(self.get_column)]\
